...
final class DeploymentCoordinator
{
    ...
    {
        Event::dispatch(new DeploymentStarted($this->queues, now()));
        ...
        if (! $completed) {
            // Timeout occurred
            $remainingJobs = $this->waiter->getRunningJobs($this->queues);

            if ($this->onTimeoutCallback) {
                ($this->onTimeoutCallback)($remainingJobs);
            }

            Event::dispatch(new DeploymentTimedOut(
                $this->queues,
                $this->timeout,
                $remainingJobs->count(),
            ));
            ...
            if ($this->shouldCancel) {
                ...
                Event::dispatch(new DeploymentCompleted(
                    $this->queues,
                    now(),
                    $remainingJobs->count(),
                ));
        ...
        Event::dispatch(new DeploymentCompleted($this->queues, now(), 0));
